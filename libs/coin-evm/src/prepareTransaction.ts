...
export const prepareCoinTransaction = async (
  ...
  if (typedTransaction.useAllAmount) {
    ...
    const gasLimit = await rpcApi.getGasEstimation(account, {
      ...typedTransaction,
  ...
  const gasLimit = await rpcApi.getGasEstimation(account, typedTransaction).catch(
...
export const prepareTransaction = async (
  ...
  const feeData = await (async () => {
    ...
    if (!transaction.feesStrategy) {
      return rpcApi.getFeesEstimation(currency);
    }

    const gasOption = transaction.gasOptions?.[transaction.feesStrategy];

    return gasOption ?? rpcApi.getFeesEstimation(currency);
  })();
...
export const prepareForSignOperation = async (
  ...
  const rpcApi = getNodeApi(account.currency);
  const nonce = await rpcApi.getTransactionCount(account.currency, account.freshAddress);
